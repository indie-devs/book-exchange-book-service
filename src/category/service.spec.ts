...
describe('Category Service', () => {
    ...
    describe('createCategory', () => {
        it('should create category when dto is correct', async () => {
            ...
            expect(await categoryService.createCategory({ name: mockCategory.name })).toBe(mockCategory)
        });
        ...
        it('should throw Bad Request Exception when prisma throw error', async () => {
            ...
            await expect(categoryService.createCategory({ name: "test"})).rejects.toThrowError(new BadRequestException(error));
        });
    });
});